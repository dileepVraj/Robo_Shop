...
downloadingApplicationCode(){
    ...
    curl -o /tmp/cart.zip https://roboshop-builds.s3.amazonaws.com/cart.zip &> /dev/null
...
unzipTheApplicationCode(){
    ...
    # unziping the downloaded cart.zip file in /app directory.
    unzip -oq /tmp/cart.zip
    validate_operation $? "Unziping application code in 'tmp/cart.zip'"
...
creatingServiceFile(){
    ...
    validate_operation $? "cart.service is created"
}
...
startingCart(){
    ...
    validate_operation $? "Enabling cart"
    systemctl start cart
    validate_operation $? "Starting cart"
...
loadingCartSchema(){
    ...
    mongosh --host $1 </app/schema/cart.js &> /dev/null
    validate_operation $? "Successfully loaded cart schema to mongo_db database"
...
# loadingCartSchema "172.31.37.70"
